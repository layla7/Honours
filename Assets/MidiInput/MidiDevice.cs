...
namespace MidiDashboard
{
    ...
    */
    ...
    public abstract class MidiDevice    // 'abstract' means MidiDevice will not be instantiated (only derived classes will)
    {
        ...
        private static void GetMidiDevices(ArrayList midiInDevices)
        {
            midiInDevices.Clear();
        ...
        static public void ConfigMidiDevices(ArrayList midiInDevices)
        {
            GetMidiDevices(midiInDevices);
        ...
        public void MidiInMsgHandler(IntPtr handle, UInt32 msgType, UInt32 instance, UInt32 param1, UInt32 param2)
        {
            ...
            switch ((MidiMsgTypes) msgType) {
                case MidiMsgTypes.MM_MIM_ERROR :
                    Debug.Log("A MIDI error was received.");
                    break;
                case MidiMsgTypes.MM_MIM_OPEN :
                    Debug.Log("The MIDI device is opened.");
                    break;
                case MidiMsgTypes.MM_MIM_CLOSE :
                    Debug.Log("The MIDI device is closed.");
                    break;
                case MidiMsgTypes.MM_MIM_DATA :
                    Debug.Log("MIDI data were received.");
                    break;
                default :
                    Debug.Log("An unidentified message was received.");
                    break;
            }
        }
    }
